document.addEventListener('DOMContentLoaded', () => {
    ...
    function displayLLMResults(llmText, skipAnimation) {
        ...
        const isDisabled = llmText.includes("LLM disabled");
        ...
        if (isDisabled) {
            // Show friendly disabled message
            llmAnalysisTextP.innerHTML = `
                <div class="ai-disabled-message" style="color: #6b7280; font-style: italic; padding: 10px; border-left: 3px solid #d1d5db; background-color: #f9fafb;">
                    <i class="fas fa-info-circle"></i> AI Analysis is currently disabled (running in lightweight mode).
                </div>`;
            if (aiLoader) aiLoader.style.display = 'none';
            return;
        }
